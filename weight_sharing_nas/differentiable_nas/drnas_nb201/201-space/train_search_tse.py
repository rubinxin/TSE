...
from tse_utils import format_input_data, fo_grad_if_possible, hyper_higher_step, hypergrad_outer
...
parser.add_argument('--higher_method' ,       type=str, choices=['tse'],   default='tse', help='Use TSE for gradients')
parser.add_argument('--higher_params' ,       type=str, choices=['arch'],   default='arch', help='Which params to update as architecture')
parser.add_argument('--higher_order' ,       type=str, choices=['first'],   default="first", help='Gradient approximation')
parser.add_argument('--higher_loop' ,       type=str, choices=['bilevel'],   default="bilevel", help='Whether to make a copy of network for the Higher rollout or not. If we do not copy, it will be as in joint training')
...
parser.add_argument('--higher_reduction_outer' ,       type=str, choices=['sum'],   default='sum', help='Reduction across the meta-batch size')
parser.add_argument('--higher_algo' ,       type=str, choices=['darts_higher'],   default=None)
...
def main():
    ...
    if args.dataset == 'cifar10':
        ...
        imagenet_path = os.path.join(os.environ["TORCH_HOME"], "PATH/ImageNet16/")
...
def train_higher(train_queue, valid_queue, network, architect, criterion, w_optimizer, a_optimizer, lr, epoch, inner_steps=100):
    ...
    for data_step, ((base_inputs, base_targets), (arch_inputs, arch_targets)) in enumerate(search_loader_iter):
        ...
        monkeypatch_higher_grads_cond = True if (args.higher_order != "first") else False
        diffopt_higher_grads_cond = True if (args.higher_order != "first") else False
        ...
        if epoch >= 10:
            for inner_step, (base_inputs, base_targets, arch_inputs, arch_targets) in enumerate(zip(all_base_inputs, all_base_targets, all_arch_inputs, all_arch_targets)):
                ...
                first_order_grad = fo_grad_if_possible(args, fnetwork, criterion, all_arch_inputs, all_arch_targets, arch_inputs, arch_targets, cur_grads, inner_step, 
                                                        data_step, 1, first_order_grad, first_order_grad_for_free_cond, logger=None)
            higher_grads, inner_rollouts = hypergrad_outer(args=args, fnetwork=fnetwork, criterion=criterion, arch_targets=arch_targets, arch_inputs=arch_inputs,
                                                ...
                                                tse=tse, inner_step=inner_step, inner_steps=inner_steps, inner_rollouts=inner_rollouts,
                                                first_order_grad_for_free_cond=True, higher_grads=higher_grads,
            ...
            avg_higher_grad = hyper_higher_step(network, inner_rollouts, higher_grads, args, data_step, logger, model_init=None, outer_iters=1, epoch=epoch)
            with torch.no_grad():  
